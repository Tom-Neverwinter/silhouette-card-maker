# Magi-Nation Duel TCG GUI Integration Module
# ============================================
...
# -----------------------------
class MagiNationPluginGUI:
    """
    GUI integration class for Magi-Nation Duel TCG plugin.

    This class provides the interface between the main Silhouette Card Maker
    GUI and the Magi-Nation Duel plugin functionality.
    """

    def __init__(self, parent_frame, plugin_manager=None):
        """
        Initialize the Magi-Nation Duel plugin GUI.
    ...
    def _create_gui(self):
        ...
        title_label = ttk.Label(
            self.main_frame,
            text="Magi-Nation Duel TCG Plugin",
            font=("Arial", 16, "bold")
        )
    ...
    def _create_scrape_tab(self):
        ...
        self.scrape_source = tk.StringVar(value="central")
        sources = [
            ("Magi-Nation Central", "central"),
            ("Magi-Nation.com", "official"),
        ...
        # Filters frame
        filters_frame = ttk.LabelFrame(scrape_frame, text="Filters", padding="5")
        ...
        type_combo = ttk.Combobox(
            ...
            values=["all", "magi", "creature", "spell", "relic"],
            state="readonly",
            width=15
        )
        ...
        # Region filter
        region_frame = ttk.Frame(filters_frame)
        ...
        region_combo = ttk.Combobox(
            ...
            values=["all", "arderial", "cald", "naroom", "orothe", "underneath", "universal"],
            state="readonly",
            width=15
        )
        ...
        # Collection name
        collection_frame = ttk.Frame(scrape_frame)
    ...
    def _create_search_tab(self):
        ...
        columns = ("name", "type", "region", "cost", "energy", "attack", "defense")
        ...
        self.search_tree.heading("region", text="Region")
        self.search_tree.heading("cost", text="Cost")
        self.search_tree.heading("energy", text="Energy")
        ...
        self.search_tree.column("region", width=100)
        self.search_tree.column("cost", width=50)
        self.search_tree.column("energy", width=50)
    ...
    def _run_scraping(self):
        ...
        try:
            ...
            if source == 'central' or source == 'all':
                self._append_to_results("Fetching from Magi-Nation Central...\n")
                central_cards = get_cards_from_magi_nation_central(card_type, max_cards)
                all_cards.extend(central_cards)
                self._append_to_results(f"Found {len(central_cards)} cards from Magi-Nation Central\n")

            if source == 'official' or source == 'all':
                self._append_to_results("Fetching from Magi-Nation.com...\n")
                official_cards = get_cards_from_magi_nation_com(card_type, max_cards)
                all_cards.extend(official_cards)
                self._append_to_results(f"Found {len(official_cards)} cards from Magi-Nation.com\n")

            if source == 'community' or source == 'all':
                self._append_to_results("Fetching from community resources...\n")
                community_cards = get_cards_from_community_resources(card_type, max_cards)
                all_cards.extend(community_cards)
                self._append_to_results(f"Found {len(community_cards)} cards from community resources\n")
            ...
            if all_cards:
                ...
                if self.fetch_images_var.get():
                    ...
                    processed = process_magi_nation_cards_batch(all_cards, self.images_dir_var.get())
    ...
    def _fetch_search_images(self):
        ...
        if selected_cards:
            processed = process_magi_nation_cards_batch(selected_cards, self.images_dir_var.get())
    ...
    def _new_collection(self):
        ...
        self.current_collection = MagiNationDeck(
    ...
    def _display_collection_info(self, collection):
        ...
        info_text += f"Regions: {', '.join(collection.regions)}\n"
    ...
    def get_plugin_info(self):
        ...
        return {
            "name": "Magi-Nation Duel TCG",
            "version": "1.0.0",
            "description": "Plugin for Magi-Nation Duel Trading Card Game deck scraping and management",
