...
def generate_pdf(
    ...
    with open(layouts_path, 'r') as layouts_file:
        ...
        with Image.open(registration_path) as reg_im:
            ...
            if len(pages) == 0:
                click.secho('[PDF] No pages were generated', fg='yellow')
                return

            # Always apply an offset so it is consistently accounted for
            saved_offset = load_saved_offset()
            if saved_offset is None:
                click.secho('[PDF] No saved offset found; applying 0,0 offset.', fg='yellow')
                pages = offset_images(pages, 0.0, 0.0, ppi)
            else:
                click.secho(
                    f"[PDF] Loaded x offset: {saved_offset.x_offset}, y offset: {saved_offset.y_offset}",
                    fg='cyan',
                )
                pages = offset_images(pages, saved_offset.x_offset, saved_offset.y_offset, ppi)

            # Save the pages array as a PDF or images
            if output_images:
...
def offset_images(images: List[Image.Image], x_offset: float, y_offset: float, ppi: int) -> List[Image.Image]:
    ...
    # x_offset and y_offset are in millimeters; convert to pixels using PPI
    # mm -> inches: mm / 25.4, then * ppi
    x_pixels = math.floor((x_offset / 25.4) * ppi)
    y_pixels = math.floor((y_offset / 25.4) * ppi)
